...
def display_results
    ...
    most_votes = $votes.max_by { |candidate, votes| votes }
    least_votes = $votes.min_by { |candidate, votes| votes }

    puts "\nCandidato mais votado: #{most_votes[0]} com #{most_votes[1]} votos"
    puts "Candidato menos votado: #{least_votes[0]} com #{least_votes[1]} votos"

    winner = most_votes[0]
    if most_votes[1] == 0
        puts "Nenhum candidato recebeu votos."
    elsif $votes.values.count(most_votes[1]) > 1
        puts "Houve um empate. Nenhum vencedor definido."
    else
        puts "Vencedor: #{winner}"
    end
end
